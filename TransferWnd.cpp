...
//<<< WiZaRd::Advanced Transfer Window Layout [Stulle]
    ...
    m_tooltipCats->SetDelayTime(TTDT_INITIAL, 1000);
...
void CTransferWnd::ShowQueueCount(UINT number)
{
    ...
    double percent = 0;
    if (number != 0 && thePrefs.GetQueueSize() != 0)
        percent = (double) 100 * number / thePrefs.GetQueueSize();

    strBuffer.Format(L"%.1f%% (%u/%u - %u %s)", percent, number, thePrefs.GetQueueSize(), theApp.clientlist->GetBannedCount(), GetResString(IDS_BANNED).MakeLower());
    GetDlgItem(IDC_QUEUECOUNT)->SetWindowText(strBuffer);
}
...
//<<< WiZaRd::Advanced Transfer Window Layout [Stulle]
        ...
        case MP_HM_OPENINC:
            _ShellExecute(NULL, _T("open"), thePrefs.GetCategory(m_isetcatmenu)->strIncomingPath,NULL, NULL, SW_SHOW);
            break;
...
CString CTransferWnd::GetCatTitle(int catid)
{
    ...
    {
        case 0 :
            return GetResString(IDS_ALL);
        case 1 :
            return GetResString(IDS_ALLOTHERS);
        case 2 :
            return GetResString(IDS_STATUS_NOTCOMPLETED);
        case 3 :
        ...
        case 8 :
            return GetResString(IDS_SEENCOMPL);
        case 10 :
            return GetResString(IDS_VIDEO);
        case 11 :
            return GetResString(IDS_AUDIO);
        case 12 :
...
//     SetWnd1Icon(w1iDownloadFiles);
void CTransferWnd::ShowSplitWindow(const bool bReDraw, const UINT dwListIDC, const bool bInitSplitted)
{
...
//>>> WiZaRd::Advanced Transfer Window Layout [Stulle]
    if (dwListIDC != IDC_UPLOADLIST)
    {
        uploadlistctrl.GetWindowRect(rcDown);
        ScreenToClient(rcDown);
        rcDown.right = rcWnd.right/* - 7*/-5; // ==> jerry : WindowSideFrameBars aligned
        rcDown.bottom = rcWnd.bottom - 20;
...
//>>> WiZaRd::Advanced Transfer Window Layout [Stulle]
void CTransferWnd::UpdateListCountTop(const EWnd2 listindex)
{
    CString strBuffer = L"";
    switch (listindex)
    {
