...
CString CreateED2kLink(const CKnownFile* f, const CString strAdditionalParameter, bool bHashset)
{
	...
	if (f->m_pAICHHashSet->GetStatus() == AICH_HASHSETCOMPLETE)
...
CString CreateED2kHashsetLink(const CKnownFile* pFile, const CString strAdditionalParameter)
{
	return CreateED2kLink(pFile,strAdditionalParameter,true);
}
