...
public:
    ...
    const Kademlia::CKadTagValueString& GetKeyword() const
    {
        return m_strKeyword;
    }
    ...
    const CSimpleKnownFileArray& GetReferences() const
    {
        return m_aFiles;
    }

    UINT GetNextPublishTime() const
    {
        return m_tNextPublishTime;
    }
    void SetNextPublishTime(UINT tNextPublishTime)
    {
        m_tNextPublishTime = tNextPublishTime;
    }

    UINT GetPublishedCount() const
    {
        return m_uPublishedCount;
    }
    void SetPublishedCount(UINT uPublishedCount)
    {
        m_uPublishedCount = uPublishedCount;
    }
    void IncPublishedCount()
    {
        m_uPublishedCount++;
    }
...
public:
    ...
    UINT GetNextPublishTime() const
    {
        return m_tNextPublishKeywordTime;
    }
    void SetNextPublishTime(UINT tNextPublishKeywordTime)
    {
...
int CAddFileThread::Run()
{
    ...
    if (m_partfile)
        Log(GetResString(IDS_HASHINGFILE) + L" \"%s\" \"%s\"", m_partfile->GetFileName(), strFilePath);
    else
        Log(GetResString(IDS_HASHINGFILE) + L" \"%s\"", strFilePath);
...
bool CSharedFileList::RemoveFile(CKnownFile* pFile, bool bDeleted)
{
    CSingleLock listlock(&m_mutWriteList);
    listlock.Lock();
    const bool bResult = m_Files_map.RemoveKey(CCKey(pFile->GetFileHash())) != FALSE;
    listlock.Unlock();
        ...
        CUpDownClient* pClient)
{
    ...
    {
        // we send a real 64 bit tag to other clients.
        {
    ...
    {
        CString strExt;
        int iExt = cur_file->GetFileName().ReverseFind(L'.');
        if (iExt != -1)
        {
    ...
    // only send verified meta data to clients
        ...
        for (int i = 0; i < ARRSIZE(_aMetaTags); ++i)
        {
            ...
            if (pTag != NULL)
            {
                ...
                else if (_aMetaTags[i].nName == FT_MEDIA_LENGTH && pTag->IsInt())
                {
                    ...
                    // eMule versions >= 0.42.4 support the media length tag with type 'integer'
                    if (uEmuleVer >= MAKE_CLIENT_VERSION(0,42,4))
                    {
                        tags.Add(new CTag(_aMetaTags[i].pszED2KName, pTag->GetInt()));
                    }
                    else
    ...
    for (int i = 0; i < tags.GetSize(); ++i)
    {
        ...
        if (uEmuleVer >= MAKE_CLIENT_VERSION(0,42,7))
...
bool CSharedFileList::ExcludeFile(CString strFilePath)
{
    ...
    // check if we implicitly share this file
...
void CSharedFileList::CheckAndAddSingleFile(const CFileFind& ff)
{
    ...
    if (_tcsicmp(szExt, L".lnk") == 0)
    {
        ...
        if (SHGetFileInfo(strFoundFilePath, 0, &info, sizeof(info), SHGFI_ATTRIBUTES) && (info.dwAttributes & SFGAO_LINK))
        {
            ...
            {
                TRACE(L"%hs: Did not share file \"%s\" - not supported file type\n", __FUNCTION__, strFoundFilePath);
                return;
            }
    ...
    if (strFoundFileName.CompareNoCase(L"thumbs.db") == 0)
    {
    ...
    if (fdate == -1)
    {
        if (thePrefs.GetVerbose())
            AddDebugLogLine(false, L"Failed to get file date of \"%s\"", strFoundFilePath);
    }
    else
        ...
        if (m_Files_map.Lookup(key, pFileInMap))
        {
            ...
            if (!pFileInMap->IsKindOf(RUNTIME_CLASS(CPartFile)) || theApp.downloadqueue->IsPartFile(pFileInMap))
            {
                if (pFileInMap->GetFilePath().CompareNoCase(toadd->GetFilePath()) != 0) /* is it actually really the same file in the same place we already share? if so don't bother too much */
                    ...
                    DebugLog(L"File shared twice, might have been a single shared file before - %s" , pFileInMap->GetFilePath());
            }
        }
        else
        {
            if (!strShellLinkDir.IsEmpty())
                DebugLog(L"Shared link: %s from %s", strFoundFilePath, strShellLinkDir);
    ...
    else
    {
                ...
                && !thePrefs.IsForbiddenFile(strFoundFileName)) //>>> WiZaRd::Remove forbidden files
        {
            ...
            TRACE(L"%hs: Did not share file \"%s\" - already hashing or temp. file\n", __FUNCTION__, strFoundFilePath);
        // SLUGFILLER: SafeHash
    }
}
...
void CSharedFileList::Save() const
{
    ...
    if (sdirfile.Open(strFullPath, CFile::modeCreate | CFile::modeWrite | CFile::shareDenyWrite | CFile::typeBinary))
    {
        try
        {
            ...
            {
                sdirfile.WriteString(L"-" + m_liSingleExcludedFiles.GetNext(pos)); // a '-' prefix means excluded
                sdirfile.Write(L"\r\n", sizeof(TCHAR)*2);
            }
...
CString CSharedFileList::GetPseudoDirName(const CString& strDirectoryName)
{
    ...
    if (!strPseudoName.IsEmpty())
    {
        ...
        ASSERT(strPseudoName.GetAt(0) == L'\\');
    ...
    if (m_mapPseudoDirNames.Lookup(strPseudoName, strDirectoryTmp))
    {
        ...
        for (iPos = 2; ; ++iPos)
        {
            strUnique.Format(L"%s_%u", strPseudoName, iPos);
            if (!m_mapPseudoDirNames.Lookup(strUnique, strDirectoryTmp))
            {
                DebugLog(L"Using Pseudoname %s for directory %s", strUnique, strDirectoryName);
    ...
    {
        DebugLog(L"Using Pseudoname %s for directory %s", strPseudoName, strDirectoryName);
...
void CSharedFileList::GetUsefulDirectories(CStringList& dirList, const bool bForAFriend) const //>>> WiZaRd::SharePermissions
{
    CKnownFile* cur_file = NULL;
    CCKey bufKey;
...
//>>> WiZaRd::SharePermissions
            // pick the proper permissions for the current directory
            bool bAdd = false;
            for (POSITION pos = thePrefs.shareddir_list.GetHeadPosition(), pos2 = thePrefs.shareddir_list_permissions.GetHeadPosition(); pos != NULL;)
            {
                ...
                if (strCurrentDir.CompareNoCase(strSharedFileDir) == 0) //found dir
                {
                    if (permission == eSP_Options)
                        permission = (uint8)thePrefs.CanSeeShares() + 1;
                    if (permission == eSP_Everybody || (permission == eSP_Friends && bForAFriend))
                        bAdd = true;
                    break;
                }
            }
