...
///----------------------------------------------------------------------------
/// <VersionHistory>
///  2012-02-12 : Added icon array by WiZaRd
...
LPCTSTR SnarlInterface::GetAppPath()
{
    ...
    if (hWnd)
    {
        HWND hWndPath = FindWindowEx(hWnd, NULL, L"static", NULL);
        if (hWndPath)
        {
...
LPCTSTR SnarlInterface::GetIconsPath()
{
    ...
    if (nLen > 0)
    {
        ...
        _tcsncat(szIconPath, L"etc\\icons\\", nLen);
    }
...
void SnarlInterface::CreateIconArray()
{
    ...
    for(int i = 0; i < TBN_DOWNLOADADDED; ++i)
    {
        ...
        _tcsncpy(szRet, szIconPath, fullLen);
        _tcsncat(szRet, szIcon, fullLen);

        szIcons[i] = szRet;
        SnarlInterface::FreeString(szRet);
    }

    SnarlInterface::FreeString(szIconPath);

    /*const CString exePath = thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR) + CString(theApp.m_pszExeName) + L".exe";
    for(int i = 0; i < TBN_DOWNLOADADDED; ++i)
    {
    	CString szIcon = L"";
    	switch (i)
    	{
    		case TBN_CHAT:
    			szIcons[i].Format(L"%s,%u", exePath, 72);
    			break;

    		case TBN_DOWNLOADFINISHED:
    			szIcons[i].Format(L"%s,%u", exePath, 182);
    			break;
    		...
    		case TBN_IMPORTANTEVENT:
    			szIcons[i].Format(L"%s,%u", exePath, 65);
    			break;

    		case TBN_NEWVERSION:
    			szIcons[i].Format(L"%s,%u", exePath, 0);
    			break;
