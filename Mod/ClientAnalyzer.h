...
// KNOW YOUR ENEMY
    bool	IsBadGuy() const;
    DWORD	GetLastSeen() const
    {
        return m_pData->dwLastSeen;
    }
    void	SetLastSeen()
    {
...
//keep bad status for some time
    bool	IsBlockedByBadTimer() const
    {
        return time(NULL) - m_pData->dwBadTimer < 7200;   // = 2h
    }
    void	SetBadTimer()
    {
...
//this tells us the reasons for the score reduce :)
    ...
    UINT	GetReaskTime() const
    {
        return m_uiReaskTime;
    }
//>>> CTempCAList - for the copy constructor
    CUpDownClient* GetParent() const
    {
        return m_pParent;
    }
    ...
    bool	GetCheckScore() const
    {
        return m_bCheckScore;
    }
    UINT	GetBadForThisSession() const
    {
        return m_uiBadForThisSession;
    }
    bool	CheckBadFlag(const UINT i) const
    {
        return (m_uiBadForThisSession & i) != 0;
    }
    CAntiLeechStruct* GetData() const
    {
        return m_pData;
    }
    const CArray<CTag*>*	GetTagList() const
    {
        return &taglist;
    }
//<<< CTempCAList
...
class CAntiLeechDataList
{
  public:
    ...
    UINT	GetBadActionCounter(const uint8 i) const
    {
        return m_uiBadActions[i];
    }
    void	IncBadActionCounter(const uint8 i)
    {
        ++m_uiBadActions[i];
    }
