                                       ...
                                       bool preview, bool bCreatePartFileCopy)
{
    ...
    try
    {
        ...
        {
            // Copy the file
            const CString strTmpPath = theApp.downloadqueue->GetOptimalTempDir(partFile->GetCategory(), thePrefs.GetPreviewCopiedArchives() ? partFile->GetFileSize() : (partFile->GetCompletedSize() + (uint64)16*1024)) + L"\\";
            tempFileName = strTmpPath + partFile->GetFileName().Mid(0, 5) + _T("-rec.tmp");
