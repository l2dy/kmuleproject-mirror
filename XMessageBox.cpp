...
public:
    ...
    UINT	GetDefaultButtonId() const
    {
        return m_nDefId;
    }
    void	SetDefaultButtonId(UINT nDefId)
    {
        m_nDefId = nDefId;
    }
...
protected:
    ...
    int		Option(EOptions id) const
    {
        return m_Options & id;
    }
    void	Set(EOptions id)
    {
        m_Options |= id;
    };
    BOOL	SetClipboardText(LPCTSTR lpszBuffer);
    void	Unset(EOptions id)
    {
        m_Options &= ~id;
    };
                ...
                XMSGBOXPARAMS * pXMB /*= NULL*/)
{
            ...
            (nStyle & MB_DONOTSHOWAGAIN))
    {
        ...
        if (xmb.lpszModule && (xmb.lpszModule[0] != L'\0'))
        {
    ...
    m_bNarrow(pXMB->dwOptions & XMSGBOXPARAMS::Narrow)				//+++1.8
{
    ...
    if (lpszMessage)
    {
        ...
        m_lpszMessage[MessageSize-1] = L'\0';
    }
    ...
    if (lpszCaption)
    {
        ...
        m_lpszCaption[MessageSize-1] = L'\0';
    }
    ...
    if (pXMB->nIdCustomButtons)
    {
        ...
        if (::LoadString(hInstanceStrings,
            ...
            m_szCustomButtons[0] = L'\0';
    }

    if ((m_szCustomButtons[0] == L'\0') && (pXMB->szCustomButtons[0] != L'\0'))
    {
        // load from string
        _tcsncpy(m_szCustomButtons, pXMB->szCustomButtons, countof(m_szCustomButtons)-1);
    }
    m_szCustomButtons[countof(m_szCustomButtons)-1] = L'\0';
    ...
    if (pXMB->nIdReportButtonCaption)
    {
        ...
        if (::LoadString(hInstanceStrings,
            ...
            m_szButtonText[eReport][0] = L'\0';
    }

    if ((m_szButtonText[eReport][0] == L'\0') && (pXMB->szReportButtonCaption[0] != L'\0'))
    {
        _tcsncpy(&m_szButtonText[eReport][0], pXMB->szReportButtonCaption, MaxButtonStringSize-1);
    }
    m_szButtonText[eReport][MaxButtonStringSize-1] = L'\0';
    ...
    const XMSGBOXPARAMS::CUserDefinedButtonCaptions& udcs)
{
    ...
    if (m_szButtonText[eReport][0] == L'\0')
        if (udcs.szReport[0])
...
void CXDialogTemplate::LoadButtonStrings()
{
    ...
    for (int index = 0; index < LAST_BUTTON; index++)
    {
        ...
        m_szButtonText[index][MaxButtonStringSize-1] = L'\0';
    }
}
...
static DWORD ReadRegistry(LPCTSTR lpszCompanyName, LPCTSTR lpszKey)
{
    _ASSERTE((lpszKey != NULL) && (lpszKey[0] != L'\0'));
    if (!lpszKey || lpszKey[0] == L'\0')
        return 0;
