...
public:
    ...
    void SetRegistryKey(LPCTSTR pszRegKey)
    {
        m_strRegKey = pszRegKey;
    }
    void SetRegistryPrefix(LPCTSTR pszPrefix)
    {
        m_strRegPrefix = pszPrefix;
    }

    void EnableHdrCtrlSortBitmaps(BOOL bUseHdrCtrlSortBitmaps = TRUE);
    void SetHdrImgList(UINT uResID, int cx, int cy, int iImages);
    int GetSortBitmapWidth() const
    {
        return m_bUseHdrCtrlSortBitmaps ? 12 : LCX_SORT_STATE_IMAGE_WIDTH;
    }
...
public:
    ...
    void OnFindNext();
    void OnFindPrev();
    int GetFindColumn() const
    {
        return m_iFindColumn;
    }
    const CString& GetFindText() const
    {
        return m_strFindText;
    }
    bool GetFindMatchCase() const
    {
        return m_bFindMatchCase;
    }
