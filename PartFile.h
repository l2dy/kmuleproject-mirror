...
public:
	...
	bool	IsCompletePart(const UINT partnumber, const bool bIgnoreBufferedData) const;
	...
	bool	GetCompletionError() const
	{
		return m_bCompletionError;
	}
	EMFileSize  GetCompletedSize() const
	{
	...
	bool	IsPausingOnPreview() const
	{
		return m_bPauseOnPreview && IsPreviewableFileType() && CanPauseFile();
	}
	...
	CAICHRecoveryHashSet* GetAICHRecoveryHashSet()
	{
		return m_pAICHRecoveryHashSet;
	}
	void	RequestAICHRecovery(UINT nPart);
	void	AICHRecoveryDataAvailable(UINT nPart);
	bool	IsAICHPartHashSetNeeded() const
	{
		return m_FileIdentifier.HasAICHHash() && !m_FileIdentifier.HasExpectedAICHHashCount() && m_bAICHPartHashsetNeeded;
	}
	void	SetAICHHashSetNeeded(bool bVal)
	{
...
public:
	uint16	GetSrcPartFrequency(const UINT part)	{return m_SrcpartFrequency[part];}
//<<< WiZaRd::Intelligent SOTN
