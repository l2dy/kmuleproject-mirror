...
//	ON_BN_CLICKED(IDC_FDCRESET, OnBnClickedResetFDC)	//>>> FDC [BlueSonicBoy]
    ...
    : CResizablePage(CFileDetailDialogName::IDD, 0)
{
    ...
    m_bSelf = false;
    m_bResetFDC = false; //>>> FDC [BlueSonicBoy]
}
...
// 	if(file)
// 		GetDlgItem(IDC_FDCRESET)->EnableWindow(file->DissimilarName());
//<<< FDC [BlueSonicBoy]
...
void CFileDetailDialogName::Localize()
{
    ...
    //GetDlgItem(IDC_FDCRESET)->SetWindowText(GetResString(IDS_FDCRESET));	//>>> FDC [BlueSonicBoy]
}
