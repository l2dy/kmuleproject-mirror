...
void CPreparedHyperText::PrepareText(const CString& sText)
{
	...
	for(int i = 0; i <= last; ++i)
	{
		...
		case space:
			...
			switch(c)
			{
				case L'h': state = http0; break;
				case L'f': state = ftp0; break;
				case L'w': state = www0; break;
				case L'm': state = mailto0; break;
				case L'e': state = ed2k0; break;
				default:
				if(!tspace(c))
					state = unknown;
			}
...
void CPreparedHyperText::AppendText(const CString& sText)
{
	...
	if(len > 60000)
	{
		...
		UINT litest = ltest.Begin() - shift;
		CKeyWord &wtest = m_KeyWords.front();
		UINT witest = wtest.Begin() - shift;
		flag = true;
...
void CHyperTextCtrl::UpdateSize(bool bRepaint){
	...
	if(rc.Width() > 5 && rc.Height() > 5)
	{
		...
		while(len>0)
		{
			...
			{
				::GetTextExtentExPoint(dc, L" " , 1, 0, NULL, NULL, &sz);
				sz.cx = 0;
				rll = ll;
			}
...
void CHyperTextCtrl::UpdateFonts()
{
	...
	LOGFONT lf;
//>>> WiZaRd
	//because the fonts are initialized AFTER this ctrl is created, 
	//I add an additional check here:
	if(m_Font->m_hObject)
//<<< WiZaRd
	{
...
// 		if (hHandCursor)
// 			m_LinkCursor = CopyCursor(hHandCursor);
// 		FreeLibrary(hModule);
// 	}
// 
// 	if (m_LinkCursor == NULL)
