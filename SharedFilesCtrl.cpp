...
void CSharedFilesCtrl::Init()
{
    ...
    InsertColumn(8, GetResString(IDS_SF_TRANSFERRED),	LVCFMT_RIGHT, 120);
...
//>>> WiZaRd::Ratio Indicator
    for (int i = 0; i < _countof(strRatioSmilies); ++i)
        m_iRatio[i] = m_ImageList.Add(CTempIconLoader(strRatioSmilies[i]));
//<<< WiZaRd::Ratio Indicator
...
void CSharedFilesCtrl::GetItemDisplayText(const CShareableFile* file, int iSubItem, LPTSTR pszText, int cchTextMax) const
{
    ...
    if (file->IsKindOf(RUNTIME_CLASS(CKnownFile)))
    {
        ...
        case 12:
            _sntprintf(pszText, cchTextMax, L"%s", GetResString(pKnownFile->IsSharedInKad() ? IDS_YES : IDS_NO));
            break;
...
void CSharedFilesCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
    ...
    int	iPowerShare = -1; //>>> WiZaRd::PowerShare
...
//>>> WiZaRd::PowerShare
            if (((CKnownFile*)pFile)->IsPowerShared())
                uCurPrioMenuItem += (MP_PSPRIOAUTO-MP_PRIOAUTO);
...
//>>> WiZaRd::PowerShare
    ...
    m_PrioMenu.EnableMenuItem(MP_PSPRIOVERYLOW, psFlag);
    m_PrioMenu.EnableMenuItem(MP_PSPRIOLOW, psFlag);
    m_PrioMenu.EnableMenuItem(MP_PSPRIONORMAL, psFlag);
    m_PrioMenu.EnableMenuItem(MP_PSPRIOHIGH, psFlag);
    m_PrioMenu.EnableMenuItem(MP_PSPRIOVERYHIGH, psFlag);
    m_PrioMenu.EnableMenuItem(MP_PSPRIOAUTO, psFlag);
    m_PrioMenu.CheckMenuRadioItem(MP_PRIOVERYLOW, MP_PSPRIOAUTO, uPrioMenuItem, 0);
...
BOOL CSharedFilesCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
    ...
    CTypedPtrList<CPtrList, CKnownFile*> selectedKnown; //>>> WiZaRd::PowerShare
    POSITION pos = GetFirstSelectedItemPosition();
    while (pos != NULL)
    {
...
//<<< Tux::RenameWithoutExtension
        ...
        case MP_PRIOAUTO:
//>>> WiZaRd::PowerShare
        case MP_PSPRIOVERYLOW:
        case MP_PSPRIOLOW:
        case MP_PSPRIONORMAL:
        case MP_PSPRIOHIGH:
        case MP_PSPRIOVERYHIGH:
        case MP_PSPRIOAUTO:
//<<< WiZaRd::PowerShare
        {
//>>> WiZaRd::PowerShare
            const bool bPS = wParam > MP_PRIOAUTO;
            if (bPS)
...
void CSharedFilesCtrl::OnLvnColumnClick(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    // Ornis 4-way-sorting
    int adder = 0;
    if (pNMListView->iSubItem >= 6 && pNMListView->iSubItem <= 8) // 6=IDS_SF_REQUESTS, 7=IDS_SF_ACCEPTS, 8=IDS_SF_TRANSFERRED
    {
        ASSERT(pNMListView->iSubItem - 6 < _countof(m_aSortBySecondValue));
...
void CSharedFilesCtrl::CreateMenues()
{
    ...
    if (m_PowerSharedMenu) VERIFY(m_PowerSharedMenu.DestroyMenu());   //>>> WiZaRd::PowerShare
...
//>>> WiZaRd::PowerShare
    ...
    m_PrioMenu.AppendMenu(MF_STRING,MP_PSPRIOVERYLOW, GetResString(IDS_POWERSHARE_PREFIX)+ L" ("+GetResString(IDS_PRIOVERYLOW)+L")", L"DOWNLOAD");
    m_PrioMenu.AppendMenu(MF_STRING,MP_PSPRIOLOW, GetResString(IDS_POWERSHARE_PREFIX)+ L" ("+GetResString(IDS_PRIOLOW)+L")", L"PRIO_LOW");
    m_PrioMenu.AppendMenu(MF_STRING,MP_PSPRIONORMAL, GetResString(IDS_POWERSHARE_PREFIX)+ L" ("+GetResString(IDS_PRIONORMAL)+L")", L"PRIO_NORMAL");
    m_PrioMenu.AppendMenu(MF_STRING,MP_PSPRIOHIGH, GetResString(IDS_POWERSHARE_PREFIX)+ L" ("+GetResString(IDS_PRIOHIGH)+L")", L"PRIO_HIGH");
    m_PrioMenu.AppendMenu(MF_STRING,MP_PSPRIOVERYHIGH, GetResString(IDS_POWERSHARE_PREFIX)+ L" ("+GetResString(IDS_PRIORELEASE)+L")", L"UPLOAD");
    m_PrioMenu.AppendMenu(MF_STRING,MP_PSPRIOAUTO, GetResString(IDS_POWERSHARE_PREFIX)+ L" ("+GetResString(IDS_PRIOAUTO)+L")", L"PRIO_AUTO");//UAP
