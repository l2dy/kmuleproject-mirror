...
#define SV_OFFSET	8	//>>> WiZaRd::SharedView Ed2kType [Avi3k]
...
//>>> WiZaRd::SharedView Ed2kType [Avi3k]
        pCurImageList->Replace(SV_OFFSET+0, CTempIconLoader(L"SearchFileType_Audio"));
        pCurImageList->Replace(SV_OFFSET+1, CTempIconLoader(L"SearchFileType_Video"));
        pCurImageList->Replace(SV_OFFSET+2, CTempIconLoader(L"SearchFileType_Picture"));
        pCurImageList->Replace(SV_OFFSET+3, CTempIconLoader(L"SearchFileType_Program"));
        pCurImageList->Replace(SV_OFFSET+4, CTempIconLoader(L"SearchFileType_Document"));
        pCurImageList->Replace(SV_OFFSET+5, CTempIconLoader(L"SearchFileType_Archive"));
        pCurImageList->Replace(SV_OFFSET+6, CTempIconLoader(L"SearchFileType_CDImage"));
        pCurImageList->Replace(SV_OFFSET+7, CTempIconLoader(L"AABCOLLECTIONFILETYPE"));
//<<< WiZaRd::SharedView Ed2kType [Avi3k]
    }
    else
    {
...
//>>> WiZaRd::SharedView Ed2kType [Avi3k]
        ASSERT(iml.GetImageCount() == SV_OFFSET); //just to be sure
        iml.Add(CTempIconLoader(L"SearchFileType_Audio", 16, 16));
        iml.Add(CTempIconLoader(L"SearchFileType_Video", 16, 16));
        iml.Add(CTempIconLoader(L"SearchFileType_Picture", 16, 16));
        iml.Add(CTempIconLoader(L"SearchFileType_Program", 16, 16));
        iml.Add(CTempIconLoader(L"SearchFileType_Document", 16, 16));
        iml.Add(CTempIconLoader(L"SearchFileType_Archive", 16, 16));
        iml.Add(CTempIconLoader(L"SearchFileType_CDImage", 16, 16));
        iml.Add(CTempIconLoader(L"SearchFileType_EmuleCollection", 16, 16));
//<<< WiZaRd::SharedView Ed2kType [Avi3k]
...
//>>> WiZaRd::SharePermissions
    m_SharePermissionsMenu.CreateMenu();
    m_SharePermissionsMenu.AddMenuTitle(NULL, true);
    m_SharePermissionsMenu.AppendMenu(MF_STRING, MP_SHAREPERM_PREF, GetResString(IDS_SECURITY) + L"(" + GetResString(IDS_OPTIONS) + L")", L"PREFERENCES");
    m_SharePermissionsMenu.AppendMenu(MF_STRING, MP_SHAREPERM_ALL, GetResString(IDS_PW_EVER), L"AAAEMULEAPP");
    m_SharePermissionsMenu.AppendMenu(MF_STRING, MP_SHAREPERM_FRIENDS, GetResString(IDS_FSTATUS_FRIENDSONLY), L"FRIEND");
    m_SharePermissionsMenu.AppendMenu(MF_STRING, MP_SHAREPERM_NONE, GetResString(IDS_PW_NOONE), L"DELETE");
//<<< WiZaRd::SharePermissions
...
//>>> WiZaRd::SharePermissions
    m_SharedFilesMenu.AppendMenu(MF_STRING | MF_SEPARATOR);
    m_SharedFilesMenu.AppendMenu(MF_STRING | MF_POPUP, (UINT_PTR)m_SharePermissionsMenu.m_hMenu, GetResString(IDS_PW_SHARE), L"SECURITY");
//<<< WiZaRd::SharePermissions
...
//>>> WiZaRd::SharePermissions
    m_ShareDirsMenu.AppendMenu(MF_STRING | MF_SEPARATOR);
    m_ShareDirsMenu.AppendMenu(MF_STRING | MF_POPUP, (UINT_PTR)m_SharePermissionsMenu.m_hMenu, GetResString(IDS_PW_SHARE), L"SECURITY");
//<<< WiZaRd::SharePermissions
}
...
//>>> WiZaRd::SharePermissions
        ...
        m_SharePermissionsMenu.CheckMenuRadioItem(MP_SHAREPERM_PREF, MP_SHAREPERM_NONE, toSet, 0);
//<<< WiZaRd::SharePermissions
...
//>>> WiZaRd::SharePermissions
        case MP_SHAREPERM_PREF:
        case MP_SHAREPERM_ALL:
...
void CSharedDirsTreeCtrl::FileSystemTreeAddChildItem(CDirectoryItem* pRoot, CString strText, bool bTopLevel)
{
    ...
    if (m_bUseIcons)
    {
        ...
        else
        {
            ...
            if (bTopLevel && shFinfo.szDisplayName[0] != L'\0')
            {
