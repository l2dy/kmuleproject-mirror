...
//>>> WiZaRd::IntelliFlush
    // changed to 0.5-20MB, we can basically set everything we want :)
    m_ctlFileBuffSize.SetRange(512, 20480, TRUE);
//<<< WiZaRd::IntelliFlush
    int iMin, iMax;
...
BOOL CPPgTweaks::OnApply()
{
    ...
    if (thePrefs.m_bExtControls != m_bExtControls)
    {
        ...
        theApp.emuledlg->sharedfileswnd->UpdateMetaDataPage();
        theApp.emuledlg->preferenceswnd->UpdateShownPages();
    }
