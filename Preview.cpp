...
BOOL CPreviewThread::Run()
{
    ...
    try
    {
        ...
        const CString strTmpPath = theApp.downloadqueue->GetOptimalTempDir(m_pPartfile->GetCategory(), thePrefs.GetPreviewCopiedArchives() ? m_pPartfile->GetFileSize() : (m_pPartfile->GetCompletedSize() + (uint64)16*1024)) + L"\\";
        CString strPreviewName = strTmpPath + m_pPartfile->GetFileName().Mid(0, 5) + _T("_preview") + strExtension;
