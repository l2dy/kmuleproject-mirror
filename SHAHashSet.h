...
public:
    ...
    CAICHHash(CFileDataIO* file)
    {
        Read(file);
    }
    CAICHHash(uchar* data)
    {
        Read(data);
    }
    CAICHHash(const CAICHHash& k1)
    {
        *this = k1;
    }
    ...
    {
        memcpy(m_abyBuffer, data, HASHSIZE);
    }
...
public:
    ...
    const CAICHHashTree* FindExistingHash(uint64 nStartPos, uint64 nSize) const
    {
        uint8 buffer = 0;
        return FindExistingHash(nStartPos, nSize, &buffer);
    }
...
//<<< WiZaRd::IPv6 [Xanatos]
    ...
    EAICHStatus		GetStatus()	const
    {
        return m_eStatus;
    }
    void			SetOwner(CKnownFile* val)
    {
        m_pOwner = val;
    }
    ...
    const CAICHHash& GetMasterHash() const
    {
        return m_pHashTree.m_Hash;
    }
    void			SetMasterHash(const CAICHHash& Hash, EAICHStatus eNewStatus);
    bool			HasValidMasterHash()
    {
        return m_pHashTree.m_bHashValid;
    }
