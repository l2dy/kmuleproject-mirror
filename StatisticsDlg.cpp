...
//<<< WiZaRd::ClientAnalyzer
    ...
    if (forceUpdate || stattree.IsExpanded(h_clients))
    {
        ...
        if (forceUpdate || stattree.IsExpanded(hclisoft))
        {
            ...
            stattree.SetItemText(clisoft[0], cbuffer);
            cbuffer.Format(_T("eD Hybrid: %i (%1.1f%%)"),	myStats[ 4],(double)100*myStats[ 4]/totalclient);
            stattree.SetItemText(clisoft[1], cbuffer);
            cbuffer.Format(_T("eDonkey: %i (%1.1f%%)"),		myStats[ 1],(double)100*myStats[ 1]/totalclient);
            stattree.SetItemText(clisoft[2], cbuffer);
            cbuffer.Format(_T("aMule: %i (%1.1f%%)"),		myStats[10],(double)100*myStats[10]/totalclient);
            stattree.SetItemText(clisoft[3], cbuffer);
            cbuffer.Format(_T("MLdonkey: %i (%1.1f%%)"),	myStats[ 3],(double)100*myStats[ 3]/totalclient);
            stattree.SetItemText(clisoft[4], cbuffer);
            cbuffer.Format(_T("Shareaza: %i (%1.1f%%)"),	myStats[11],(double)100*myStats[11]/totalclient);
            stattree.SetItemText(clisoft[5], cbuffer);
            cbuffer.Format(_T("eM Compat: %i (%1.1f%%)"),	myStats[ 5],(double)100*myStats[ 5]/totalclient);
            stattree.SetItemText(clisoft[6], cbuffer);
...
//>>> WiZaRd::Detect UDP problem clients
        cbuffer.Format(L"%s: %u (%.2f%%)", GetResString(IDS_UDP_DISABLED), myStats[19], (double)100*myStats[19]/totalclient);
...
void CStatisticsDlg::CreateMyTree()
{
    ...
    for (int i = 0; i<_countof(up_ssb); i++)
        up_ssb[i] = stattree.InsertItem(GetResString(IDS_FSTAT_WAITING), hup_ssb);
    for (int i = 0; i<_countof(up_ssessions); i++)
    ...
    for (int i = 0; i<_countof(up_tsb); i++)
    ...
    for (int i = 0; i<_countof(up_tsessions); i++)
    ...
    for (int i = 0; i<_countof(down_S); i++)
    ...
    for (int i = 0; i<_countof(down_ssessions); i++)
    ...
    for (int i = 0; i<_countof(down_T); i++)
    ...
    for (int i = 0; i<_countof(down_tsessions); i++)
...
//<<< WiZaRd::ClientAnalyzer
    ...
    for (int i = 0; i<_countof(conn_sg); i++)
    ...
    for (int i = 0; i<_countof(conn_su); i++)
    ...
    for (int i = 0; i<_countof(conn_sd); i++)
    ...
    for (int i = 0; i<_countof(conn_tg); i++)
    ...
    for (int i = 0; i<_countof(conn_tu); i++)
    ...
    for (int i = 0; i<_countof(conn_td); i++)
    ...
    for (int i = 0; i<_countof(tvitime); i++)
    ...
    for (int x = 0; x<_countof(time_aap_hup); x++)
    {
        ...
        for (int i = 0; i<_countof(time_aap_up_dc[x]); i++)
        ...
        for (int i = 0; i<_countof(time_aap_up_dp[x]); i++)
        ...
        for (int i = 0; i<_countof(time_aap_down[x]); i++)
        ...
        for (int i = 0; i<_countof(time_aap_down_dc[x]); i++)
    ...
    for (int i = 0; i<_countof(clisoft); i++)
    ...
    for (int i = 0; i<_countof(clinet); i++)
    ...
    for (int i = 0; i<_countof(cliport); i++)
    ...
    for (int i = 0; i < _countof(clifirewalled); i++)
    ...
    for (int i = 0; i<_countof(cligen)-3; i++)
    ...
    for (int i = 0; i<_countof(shar_r); i++)
    ...
    if (g_pfnPrevCrtAllocHook)
    {
        h_debug = stattree.InsertItem(_T("Debug info"));
        stattree.SetItemData(h_debug,0);
        h_blocks = stattree.InsertItem(_T("Blocks"),h_debug);
        stattree.SetItemData(h_blocks,1);
        debug1 =  stattree.InsertItem(_T("Free"),h_blocks);
        stattree.SetItemData(debug1,1);
        debug2 =  stattree.InsertItem(_T("Normal"),h_blocks);
        stattree.SetItemData(debug2,1);
        debug3 =  stattree.InsertItem(_T("CRT"),h_blocks);
        stattree.SetItemData(debug3,1);
        debug4 =  stattree.InsertItem(_T("Ignore"),h_blocks);
        stattree.SetItemData(debug4,1);
        debug5 =  stattree.InsertItem(_T("Client"),h_blocks);
        stattree.SetItemData(debug5,1);
    ...
    stattree.SetItemState(h_Analyzer, TVIS_BOLD, TVIS_BOLD); //>>> WiZaRd::ClientAnalyzer
