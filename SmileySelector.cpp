...
BOOL CSmileySelector::Create(CWnd *pWndParent, const RECT *pRect, CEdit *pwndEdit)
{
    ...
    for (int i = 0; i < _countof(g_aSmilies); i++)
    {
        iml.Add(CTempIconLoader(g_aSmilies[i].pszResource, sizSmiley.cx, sizSmiley.cy));
        m_iBitmaps++;
    }
...
//#ifdef SHOW_SMILEY_SHORTCUTS
    ...
    for (int i = 0; i < _countof(g_aSmilies); i++)
    {
        strStrings += g_aSmilies[i].pszSmilies;
        strStrings += _T('\001');
    }
    strStrings += _T('\001');
...
//#endif
    ...
    for (int i = 0; i < m_iBitmaps; i++)
    {
        ...
        tb[i].dwData = (DWORD_PTR)&g_aSmilies[i];
//#ifdef SHOW_SMILEY_SHORTCUTS
        tb[i].iString = i;	// Looks interesting!! -> Smilies with shortcuts below
//#else
//        tb[i].iString = -1;
//#endif
    }
...
BOOL CSmileySelector::OnCommand(WPARAM wParam, LPARAM lParam)
{
    ...
    if (m_pwndEdit != NULL && iCmd >= SMSEL_CMD_START && iCmd < SMSEL_CMD_START + iButtons)
    {
        ...
        strInsert += g_aSmilies[iCmd - SMSEL_CMD_START].pszSmilies;
...
void CSmileySelector::OnTbnSmileyGetInfoTip(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    if (pNMTBGIT->cchTextMax)
    {
        ...
        {
            _sntprintf(pNMTBGIT->pszText, pNMTBGIT->cchTextMax, _T(" %s "), g_aSmilies[pNMTBGIT->iItem - SMSEL_CMD_START].pszSmilies);
            pNMTBGIT->pszText[pNMTBGIT->cchTextMax - 1] = L'\0';
        }
    }
    *pResult = 0;
}
