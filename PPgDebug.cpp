...
void CPPgDebug::DoDataExchange(CDataExchange* pDX)
{
    ...
    if (!m_bInitializedTreeOpts)
    {
        ...
        if (piml)
        {
            HICON hIcon = theApp.LoadIcon(_T("StatsClients"));
            if (hIcon)
            {
...
#define	ADD_INTEGER_ITEM(idx, label, group) \
        ...
        ADD_DETAIL_ITEM(0, _T("TCP"), m_htiClient);
        ADD_DETAIL_ITEM(1, _T("UDP (eD2K)"), m_htiClient);
        ADD_DETAIL_ITEM(2, _T("UDP (Kad)"), m_htiClient);
...
#define	SET_INTEGER_OPT(idx, var) \
    ...
    SET_DETAIL_OPT(0, thePrefs.m_iDebugClientTCPLevel);
    SET_DETAIL_OPT(1, thePrefs.m_iDebugClientUDPLevel);
    SET_DETAIL_OPT(2, thePrefs.m_iDebugClientKadUDPLevel);
...
#define	GET_INTEGER_OPT(idx, opt) \
    ...
    GET_DETAIL_OPT(0, thePrefs.m_iDebugClientTCPLevel);
    GET_DETAIL_OPT(1, thePrefs.m_iDebugClientUDPLevel);
    GET_DETAIL_OPT(2, thePrefs.m_iDebugClientKadUDPLevel);
