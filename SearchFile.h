...
public:
    ...
    bool	IsServerUDPAnswer() const
    {
        return m_bServerUDPAnswer;
    }
    UINT	AddSources(UINT count);
    UINT	GetSourceCount() const;
    UINT	AddCompleteSources(UINT count);
    UINT	GetCompleteSourceCount() const;
    ...
    void	SetClientID(UINT nClientID)
    {
    ...
    void	SetClientPort(uint16 nPort)
    {
        m_nClientPort = nPort;
    }
    UINT	GetClientServerIP() const
    {
        return m_nClientServerIP;
    }
    void	SetClientServerIP(UINT uIP)
    {
        m_nClientServerIP = uIP;
    }
    uint16	GetClientServerPort() const
    {
        return m_nClientServerPort;
    }
    void	SetClientServerPort(uint16 nPort)
    {
        m_nClientServerPort = nPort;
    }
    int		GetClientsCount() const
    {
        return ((GetClientID() && GetClientPort()) ? 1 : 0) + m_aClients.GetSize();
    }
    void	SetKadPublishInfo(UINT dwVal)
    {
        m_nKadPublishInfo = dwVal;
    }
    UINT	GetKadPublishInfo() const
    {
        return m_nKadPublishInfo;    // == TAG_PUBLISHINFO
    }
    bool	DidFoundMultipleAICH() const
    {
        return m_bMultipleAICHFound;
    }
    void	SetFoundMultipleAICH()
    {
        m_bMultipleAICHFound = true;
    }

    // Spamfilter
    void	SetNameWithoutKeyword(CString strName)
    {
        m_strNameWithoutKeywords = strName;
    }
    CString	GetNameWithoutKeyword()	const
    {
        return m_strNameWithoutKeywords;
    }
    void	SetSpamRating(UINT nRating)
    {
        m_nSpamRating = nRating;
    }
    UINT	GetSpamRating() const
    {
        return m_nSpamRating;
    }
    ...
    // GUI helpers
    CSearchFile* GetListParent() const
    {
        return m_list_parent;
    }
    void		 SetListParent(CSearchFile* parent)
    {
        m_list_parent = parent;
    }
    UINT		 GetListChildCount() const
    {
        return m_list_childcount;
    }
    void		 SetListChildCount(int cnt)
    {
        m_list_childcount = cnt;
    }
    ...
    {
        return m_list_bExpanded;
    }
    void		 SetListExpanded(bool val)
    {
        m_list_bExpanded = val;
    }
    ...
    struct SClient
    {
        ...
        SClient(UINT nIP, uint16 nPort, UINT nServerIP, uint16 nServerPort)
        {
            m_nIP = nIP;
            m_nPort = nPort;
    ...
    {
        m_aClients.Add(client);
    }
    const CSimpleArray<SClient>& GetClients() const
    {
        return m_aClients;
    }
    ...
    struct SServer
    {
        ...
        SServer(UINT nIP, uint16 nPort, bool bUDPAnswer)
        {
            m_nIP = nIP;
            m_nPort = nPort;
            m_uAvail = 0;
    ...
    {
        m_aServers.Add(server);
    }
    const CSimpleArray<SServer>& GetServers() const
    {
        return m_aServers;
    }
    SServer& GetServerAt(int iServer)
    {
        return m_aServers[iServer];
    }

    void	AddPreviewImg(CxImage* img)
    {
        m_listImages.Add(img);
    }
    const CSimpleArray<CxImage*>& GetPreviews() const
    {
        return m_listImages;
    }
    bool	IsPreviewPossible() const
    {
        return m_bPreviewPossible;
    }
    void	SetPreviewPossible(bool in)
    {
        m_bPreviewPossible = in;
    }
    ...
    EKnownType GetKnownType() const
    {
        return m_eKnown;
    }
    void SetKnownType(EKnownType eType)
    {
        m_eKnown = eType;
    }
...
private:
    ...
    UINT	m_nSearchID;
    UINT	m_nClientServerIP;
    uint16	m_nClientServerPort;
    UINT	m_nKadPublishInfo;
