...
#pragma once
#pragma warning(disable:4702) //code not reachable
#include <list>
#include <vector>
#pragma warning(default:4702) //code not reachable
...
public:
    ...
    void Execute();
    bool operator < (const CHyperLink& Arg) const
    {
    ...
    void SetBegin(uint16 m_iInBegin)
    {
        m_iBegin = m_iInBegin;
    }
    void SetEnd(uint16 m_iInEnd)
    {
        m_iEnd = m_iInEnd;
    }
...
public:
    ...
    bool operator< (const CKeyWord& Arg) const
    {
        return m_iEnd < Arg.m_iEnd;
    }
    UINT Begin() const
...
public:
    ...
    std::list<CHyperLink>& GetLinks()
    {
        return m_Links;
    }
    std::list<CKeyWord>& GetKeywords()
    {
        return m_KeyWords;
    }
...
protected:
    ...
    bool tspace(TCHAR c)
    {
        return _istspace((_TUCHAR)c) || /*c < L' ' || */c == L';' || c == L'!';
    }
...
public:
    ...
    void SetLinkCursor(HCURSOR LinkCursor)
    {
    ...
    void SetDefaultCursor(HCURSOR DefaultCursor)
    {
