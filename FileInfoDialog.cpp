...
public:
    ...
    BOOL Initialize()
    {
        if (!m_bInitialized)
        {
            ...
            CString strPath = theApp.GetProfileString(MOD_VERSION_PLAIN, _T("MediaInfo_MediaInfoDllPath"), _T("MEDIAINFO.DLL"));
...
bool CGetMediaInfoThread::GetMediaInfo(HWND hWndOwner, const CShareableFile* pFile, SMediaInfo* mi, bool bSingleFile)
{
    ...
    //
    bool bIsAVI = false;
    if (theApp.GetProfileInt(MOD_VERSION_PLAIN, _T("MediaInfo_RIFF"), 1))
    {
        try
        {
    ...
    //
    bool bIsRM = false;
    if (theApp.GetProfileInt(MOD_VERSION_PLAIN, _T("MediaInfo_RM"), 1))
    {
        try
        {
...
#ifdef HAVE_WMSDK_H
    bool bIsWM = false;
    if (theApp.GetProfileInt(MOD_VERSION_PLAIN, _T("MediaInfo_WM"), 1))
    {
        try
        {
    ...
    //
    if (theApp.GetProfileInt(MOD_VERSION_PLAIN, _T("MediaInfo_ID3LIB"), 1) &&
