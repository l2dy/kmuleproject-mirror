...
int CAICHSyncThread::Run()
{
    ...
    try
    {
        if (file.GetLength() >= 1)
        {
			...
			UINT nHashCount = 0;
			UINT nHashCountSize = 0;
            while (file.GetPosition() < nExistingSize)
            {
				aKnown2HashsFilePos.Add(file.GetPosition());
				aKnown2Hashs.Add(CAICHHash(&file));
                nHashCount = file.ReadUInt32();
				nHashCountSize = nHashCount*CAICHHash::GetHashSize();
				if (file.GetPosition() + nHashCountSize > nExistingSize)
                    AfxThrowFileException(CFileException::endOfFile, 0, file.GetFileName());

				// skip the rest of this hashset
                file.Seek(nHashCountSize, CFile::current);
    ...
    // those that don't are added to the hashinglist
                    ...
                    if (current_hash == pCurFile->GetFileIdentifier().GetAICHHash())
                    {
                        ...
                        if (!pCurFile->GetFileIdentifier().HasExpectedAICHHashCount())
                        {
                            if (bDbgMsgCreatingPartHashs)
                            {
                                ...
                                DebugLogWarning(L"Missing AICH Part Hashsets for known files - maybe upgrading from earlier version. Creating them out of full AICH Recovery Hashsets, shouldn't take too long");
                            }
                            ...
                            {
                                ASSERT(0);
                                DebugLogError(L"Failed to load full AICH Recovery Hashset - known2.met might be corrupt. Unable to create AICH Part Hashset - %s", pCurFile->GetFileName());
                            }
                            else
                            {
                                if (!pCurFile->GetFileIdentifier().SetAICHHashSet(tempHashSet))
                                {
                                    DebugLogError(L"Failed to create AICH Part Hashset out of full AICH Recovery Hashset - %s", pCurFile->GetFileName());
                                    ASSERT(0);
                                }
                                ASSERT(pCurFile->GetFileIdentifier().HasExpectedAICHHashCount());
                            }
                        }
                        //theApp.QueueDebugLogLine(false, L"%s - %s", current_hash.GetString(), pCurFile->GetFileName());
                        break;
                    }
                }
            ...
            (!thePrefs.IsRememberingDownloadedFiles() || thePrefs.DoPartiallyPurgeOldKnownFiles()))
    {
        ...
        try
        {
            ...
            while (file.GetPosition() < nExistingSize)
            {
                ...
                {
					file.Seek(nHashCountSize, CFile::current);
					if(!CAICHRecoveryHashSet::AddStoredAICHHash(aichHash, nCurrentHashsetPos))
					{				
						++nPurgeCount;
						++nPurgeBecauseDup;						
					}
                ...
                {
					if(!CAICHRecoveryHashSet::AddStoredAICHHash(aichHash, posWritePos))
					{
						file.Seek(nHashCountSize, CFile::current);
						++nPurgeCount;
						++nPurgeBecauseDup;						
			...
			if(nPurgeCount)
				theApp.QueueDebugLogLine(false, L"Cleaned up known2.met, removed %u hashsets, purged %u hashsets of old known files and %u duplicate hash entries (%s)", nPurgeCount - nPurgeBecauseOld - nPurgeBecauseDup, nPurgeBecauseOld, nPurgeBecauseDup, CastItoXBytes(nExistingSize - file.GetLength()));
    ...
    {
        // remember (/index) all hashes which are stored in the file for faster checking lateron
		for (int i = 0; i < aKnown2Hashs.GetCount(); i++)
	...
	if (!m_liToHash.IsEmpty())
    {
        ...
        // let first all normal hashing be done before starting out synced hashing
        ...
        for (POSITION pos = m_liToHash.GetHeadPosition(); pos != 0; ++cDone)
        {
            ...
            if (!pCurFile->CreateAICHHashSetOnly())
                theApp.QueueDebugLogLine(false, L"Failed to create AICH Hashset while sync. for file %s", pCurFile->GetFileName());
        }
    ...
    theApp.QueueDebugLogLine(false, L"AICHSyncThread finished");
    return 0;
}
...
bool CAICHSyncThread::ConvertToKnown2ToKnown264(CSafeFile* pTargetFile)
{
    ...
    try
    {
        ...
        while (oldfile.GetPosition() < oldfile.GetLength())
        {
			...
			nHashCountSize = nHashCount*CAICHHash::GetHashSize();
			if (oldfile.GetPosition() + nHashCountSize > oldfile.GetLength())
                AfxThrowFileException(CFileException::endOfFile, 0, oldfile.GetFileName());

			BYTE* buffer = new BYTE[nHashCountSize];
            oldfile.Read(buffer, nHashCountSize);
            pTargetFile->Write(aichHash.GetRawHash(), CAICHHash::GetHashSize());
            pTargetFile->WriteUInt32(nHashCount);
            pTargetFile->Write(buffer, nHashCountSize);
            delete[] buffer;
        }
        pTargetFile->Flush();
        oldfile.Close();
