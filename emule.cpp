...
void ShowUnicowsError()
{
    ...
    CStringA msg = "";
    msg.Format("This %s version requires the \"Microsoft(R) Layer for Unicode(TM) on Windows(R) 95/98/ME Systems\".\r\n"
               "\r\n"
               ...
               "\r\n"
               "\r\n"
               "After downloading the MSLU package, run the \"unicows.exe\" program and specify your %s installation folder "
               "where to place the extracted files from the package.\r\n"
               "\r\n"
               "Ensure that the file \"unicows.dll\" was placed in your %s installation folder and start %s again.",
               MOD_VERSION_PLAIN, MOD_VERSION_PLAIN, MOD_VERSION_PLAIN, MOD_VERSION_PLAIN);
    MessageBoxA(NULL,
                msg,
                CStringA(MOD_VERSION_PLAIN),
...
extern "C" HMODULE __stdcall ExplicitPreLoadUnicows()
{
    ...
    // NOTE: Do *NOT* use eMule's localization functions here!
    CStringA msg = "";
    msg.Format("This %s version (Unicode, MSLU, shared MFC) does not run with this version of Windows.", MOD_VERSION_PLAIN);
    MessageBoxA(NULL,
                msg,
                MOD_VERSION_PLAIN,
                MB_ICONSTOP | MB_SYSTEMMODAL | MB_SETFOREGROUND);
...
// object files which are used to link kMule.exe, the linker may or may not
// throw some errors about 'safeseh'. Those errors have to get resolved until
// the linker is capable of linking kMule.exe *with* "/SafeSEH".
//
...
void InitSafeSEH()
{
    ...
    {
        AfxMessageBox(L"kMule.exe was not linked with /SafeSEH!", MB_ICONSTOP);
    }
}
...
CemuleApp theApp(MOD_VERSION_PLAIN);
...
//>>> Tux::Why do we do this?
    /*
    #ifndef _BETA
        if (GetProfileInt(MOD_VERSION_PLAIN, _T("CreateCrashDump"), 0))
    #endif
    */
//<<< Tux::Why do we do this?
    ...
    {
        if (GetProfileInt(MOD_VERSION_PLAIN, _T("CheckComctl32"), 1)) // just in case some user's can not install that package and have to survive without it..
        {
    ...
    {
        if (GetProfileInt(MOD_VERSION_PLAIN, _T("CheckShell32"), 1)) // just in case some user's can not install that package and have to survive without it..
        {
...
{
    bool bIgnoreRunningInstances = (GetProfileInt(MOD_VERSION_PLAIN, _T("IgnoreInstances"), 0) != 0);
    ...
    UINT uTcpPort = GetProfileInt(MOD_VERSION_PLAIN, _T("Port"), DEFAULT_TCP_PORT_OLD);
...
//<<< WiZaRd::Recode
    ...
    {
        m_dwPublicIP = 0;
        AddDebugLogLine(DLP_VERYLOW, false, L"Deleted public IP");
    }
}
    ...
    if (pszSkinProfile != NULL && pszSkinProfile[0] != L'\0')
    {
        ...
        if (szSkinResource[0] != L'\0')
        {
            ...
            if (ExpandEnvironmentStrings(szSkinResource, szExpSkinRes, _countof(szExpSkinRes)) != 0)
            {
                ...
                szSkinResource[_countof(szSkinResource)-1] = L'\0';
            }
            ...
            if (PathIsRelative(szSkinResource))
            {
                ...
                szSkinResFolder[_countof(szSkinResFolder)-1] = L'\0';
            ...
            else
            {
                ...
                szFullResPath[_countof(szFullResPath)-1] = L'\0';
            }
...
BOOL WINAPI ConsoleCtrlHandler(DWORD dwCtrlType)
{
    ...
    if (thePrefs.GetDebug2Disk())
    {
        ...
        else
        {
            ...
            szCtrlType[_countof(szCtrlType) - 1] = L'\0';
...
CString  CemuleApp::GetClientVersionStringBase(const bool bDebug) const
{
