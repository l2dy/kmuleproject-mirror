...
public:
    struct SLookupHistoryEntry
    {
        ...
        {
            return m_dwAskedContactsTime != 0 || m_dwAskedSearchItemTime != 0 || m_bForcedInteresting;
        }
    };
    ...
    void			SetGUIName(const CKadTagValueString& sName)
    {
        m_sGUIName = sName;
    }
    ...
    {
        return m_bSearchStopped;
    }
    bool			IsSearchDeleted() const
    {
        return m_bSearchDeleted;
    }

    CArray<SLookupHistoryEntry*>& GetHistoryEntries()
    {
        return m_aIntrestingHistoryEntries;
    }
