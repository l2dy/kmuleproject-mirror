    ...
    : CListCtrlItemWalk(this)
{
    ...
    m_eFileSizeFormat = (EFileSizeFormat)theApp.GetProfileInt(MOD_VERSION_PLAIN, _T("SearchResultsFileSizeFormat"), fsizeDefault);
    SetSkinKey(L"SearchResultsLv");
}

void CSearchListCtrl::OnDestroy()
{
    theApp.WriteProfileInt(MOD_VERSION_PLAIN, _T("SearchResultsFileSizeFormat"), m_eFileSizeFormat);
    __super::OnDestroy();
}
...
void CSearchListCtrl::Init(CSearchList* in_searchlist)
{
    ...
    InsertColumn(15,GetResString(IDS_CMT_FAKE),		LVCFMT_LEFT,  250); //>>> WiZaRd::AntiFake
...
void CSearchListCtrl::Localize()
{
    ...
    for (int icol=0; icol<pHeaderCtrl->GetItemCount(); icol++)
    {
        ...
        case 12:
            ...
            break;
        case 13:
            strRes = GetResString(IDS_KNOWN);
            break;
        case 14:
            strRes = GetResString(IDS_AICHHASH);
            break;
//>>> WiZaRd::AntiFake
        case 15:
            strRes = GetResString(IDS_CMT_FAKE);
            break;
//<<< WiZaRd::AntiFake
        }
...
void CSearchListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
    ...
    if (m_SearchFileMenu.InsertMenu(MP_REMOVESELECTED, MF_STRING | MF_ENABLED, MP_MARKASSPAM, (bContainsNotSpamFile || iSelected == 0) ? GetResString(IDS_MARKSPAM) : GetResString(IDS_MARKNOTSPAM), _T("Spam")))
    {
...
//>>> WiZaRd::SearchColors
    m_crSearchResultDownloading		= RGB(255,128,0);		//orange
    m_crSearchResultDownloadStopped	= RGB(255,0,0);			//red
    m_crSearchResultShareing		= RGB(0,192,0);			//green
    m_crSearchResultKnown			= RGB(0,128,0);			//dark green
    m_crSearchResultCancelled		= RGB(192,192,0);		//dark yellow
//<<< WiZaRd::SearchColors
